...
int init_upd_socket(void) {
    ...
    UDP_SERVER.sin_port = htons(DOWNLINK_SERVER_PORT);
    if ((ret = inet_pton(AF_INET, DOWNLINK_SERVER_IP, &UDP_SERVER.sin_addr)) <= 0) {
        fprintf(stderr,
 ...
 * @return
 *
 * @TODO Replace `char buffer` with more fitting data type
 */
